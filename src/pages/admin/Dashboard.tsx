...
import { 
  ...
  IconDownload, IconCalendar, IconPlus, IconCheck, IconSearch,
  IconUsers, IconShield, IconBooks, IconChevronDown, IconChevronRight, IconTrash
} from '@tabler/icons-react';
import { exportToExcel } from '../../utils/exportExcel';
import { cn, Button } from '../../components/ui/Button';
import { Modal } from '../../components/ui/Modal';
import { ConfirmModal } from '../../components/ui/ConfirmModal';
import { ExpenseForm } from '../superadmin/ExpenseForm';
import { DataListModal } from '../superadmin/DataListModal';
...
export const AdminDashboard = () => {
  ...
  const filterByDate = (dateStr: string) => {
    ...
    if (dateFilter === 'week') {
      const weekAgo = new Date(dateToday);
      weekAgo.setDate(weekAgo.getDate() - 7);
      return date >= weekAgo;
    }
    if (dateFilter === 'month') {
      return date.getMonth() === dateToday.getMonth() && date.getFullYear() === dateToday.getFullYear();
    }
    if (dateFilter === 'last_month') {
      const lastMonth = new Date(dateToday);
  ...
  const branchStudents = displayStudents;
  const branchGroups = displayGroups;
  const filteredStudentsForPayment = selectedGroupId ? branchStudents.filter(s => s.groupId === selectedGroupId && (s.coursePrice - s.paidAmount) > 0) : [];
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-text-muted font-medium mt-1">Filialingizning umumiy va moliyaviy holati nazorati</p>
        </div>
        ...
        <div className="flex flex-wrap xl:flex-nowrap items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <Button onClick={() => setAddPaymentModalOpen(true)} className="gap-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full px-5 h-11 font-bold shadow-md shadow-emerald-500/20 !bg-none border-transparent">
              <IconPlus size={18} stroke={2.5} /> To'lov kiritish
            </Button>
            
            <Button variant="danger" onClick={() => setExpenseModalOpen(true)} className="gap-2 rounded-full px-5 h-11 font-bold shadow-md border-transparent">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pb-4">
        ...
        <div 
          ...
          className="flex items-center gap-4 bg-bg-card border border-border/40 shadow-sm hover:shadow-md px-5 py-4 rounded-2xl cursor-pointer transition-all hover:-translate-y-0.5 w-full"
        >
          <div className="w-12 h-12 rounded-full bg-sky-50 dark:bg-sky-500/10 flex flex-shrink-0 items-center justify-center text-sky-600 dark:sky-400">
        ...
        >
          ...
          <div className="overflow-hidden">
            <p className="text-xs text-text-muted font-bold uppercase tracking-wider truncate">O'qituvchi va Xodimlar</p>
                      ...
                      transactions.slice((currentPage - 1) * 10, currentPage * 10).map((t) => (
                        <TableRow key={t.id} className="hover:bg-bg-hover/50">
                          ...
                          <TableCell className="text-text-secondary text-sm">
                            <span className="opacity-90">{t.addedBy}</span>
                          </TableCell>
                            ...
                            return (
                              <TableRow key={s.id} className="hover:bg-bg-hover/50 bg-bg-card">
                                ...
                                <TableCell className="text-right">
                                  ...
                                  }} className="border-accent text-accent hover:bg-accent hover:text-white h-8 text-xs">
                                    To'lov
                                  </Button>
                                </TableCell>
                              </TableRow>
      ...
      <Modal isOpen={addPaymentModalOpen} onClose={() => setAddPaymentModalOpen(false)} title="To'lov kiritish">
        <form onSubmit={handleAddPaymentSubmit} className="space-y-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            >
              <option value="" className="bg-bg-card text-text-primary">O'quvchini tanlang</option>
              {filteredStudentsForPayment.map(s => {
                const debt = s.coursePrice - s.paidAmount;
                return (
                  <option key={s.id} value={s.id} disabled={debt <= 0} className="bg-bg-card text-text-primary">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-border mt-4">
            ...
            <Button type="submit" disabled={!selectedStudentId || !paymentAmount}>Saqlash</Button>
          </div>
        </form>
      </Modal>
      ...
      <DataListModal 
        ...
        branches={[]} // Branches not needed for Admin view
      />
    </div>
  );
};
